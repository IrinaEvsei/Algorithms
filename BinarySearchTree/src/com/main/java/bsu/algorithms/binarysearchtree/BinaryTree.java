...
//            node.setLeft(tmp);
//
//            node.setCountNodeLeft(node.getLeft().getCountNodeLeft());//количество потомков справа, я правую чатсь не трогал значит не меняю
//            node.getRight().setCountNodeLeft(node.getRight().getCountNodeRight());//у нода беру левый и сетаю справа
//            node.getRight().setCountNodeRight(node.getCountNodeRight());//слева
//            node.setCountNodeRight(node.getRight().getCountNodeRight() + node.getRight().getCountNodeLeft() + 1);//сетаю самому ноду
//        }
//    }
    ...
    public void rotationLeftNode(TreeNode node){
        if (node == null) return;
        if (node.getRight() != null){
            TreeNode tmp = new TreeNode(node.getNumber());
            node.setNumber(node.getRight().getNumber());
